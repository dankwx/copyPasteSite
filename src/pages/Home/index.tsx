...
import { QueryDocumentSnapshot, DocumentData, doc, getDocs } from 'firebase/firestore';
import {
  getFirestore,
  collection,
  query,
...
export default function Home() {
  ...
  // Função para verificar se a string é um link
  const isLink = (str: string): boolean => {
    // Expressão regular simples para verificar se a string se parece com uma URL
    const pattern = /^(https?:\/\/)?([\w.]+)\.([a-z]{2,6}\.?)(\/[\w.]*)*\/?$/i;
    return pattern.test(str);
  };

  // Componente LinkWrapper para tratar links
  ...
  const handleDeleteMessage = async (message: string) => {
    try {
      // Criar a referência da coleção
      const messagesCollection = collection(db, "messages");

      // Criar a query
      const q = query(messagesCollection, orderBy("timestamp", "desc"));

      // Obter o snapshot da query
      const querySnapshot: QuerySnapshot<DocumentData> = await getDocs(q);

      // Usando o tipo QueryDocumentSnapshot<DocumentData>
      const docToDelete: QueryDocumentSnapshot<DocumentData> | undefined = querySnapshot.docs.find(doc => doc.data().message === message);

      // Excluir o documento se encontrado
      if (docToDelete) {
